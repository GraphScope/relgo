...
\todo{Further add contents based on the following drafts.}

In the realms of data management and analytics, relational databases have long been the bedrock of structured data storage and retrieval, empowering a plethora of applications across domains ranging from finance to healthcare and beyond. The ubiquity of these databases has been supported by the advent of Structured Query Language (SQL), a standardized language that has been adopted widely by various relational database management systems (RDBMS) for managing data through well-defined schema-based operations. SQL has been pivotal in the progression of data management, enabling the efficient manipulation of complex data structures and fostering interoperability between disparate systems.

Despite its resounding success and broad adoption, SQL has its limitations, particularly when it comes to representing and querying intricately linked data. Consider, for instance, the relational constructs of $\relation{Person}$ and $\relation{Knows}$, the latter symbolizing a many-to-many relationship between instances of the former. Constructing a SQL query to retrieve a group of four persons who are all mutually acquainted is not a straightforward endeavor, potentially leading to a cumbersome and complex SQL command.

Conversely, such a scenario could be elegantly and succinctly addressed using graph query languages, exemplified by Cypher, a language where queries are expressed as intuitive and concise graph pattern matches. This discrepancy between the relational and graph querying paradigms has given rise to the innovative SQL/Property Graph Queries (SQL/PGQ), an extension proposed as part of SQL 2023. SQL/PGQ is designed to amalgamate the extensive capabilities of SQL with the inherent benefits of graph query operations. With SQL/PGQ, it is now possible to define and query graphs within the comfort of SQL expressions, transforming otherwise complex relational queries—characterized by multiple joins—into simpler and more intuitive graph queries.

SQL/PGQ introduces the representation of graphs as views, where vertices and edges are correspondingly depicted as tables. This representation is instrumental in allowing graph queries and relational queries to be articulated within a single SQL/PGQ statement. The elegance and potential of SQL/PGQ are illustrated in \refex{introduction:sqlpgq}, where a graph view named \textbf{friendship\_graph} is constructed, delineating the intricate connections between individuals and encapsulating the relational data within the graph abstraction.

The SQL/PGQ standard, while a significant leap forward, primarily addresses language constructs. A discernible gap exists in the theoretical landscape, particularly in analyzing, transforming, and optimizing queries that interlace relational and graph semantics. This gap is palpable in the absence of a unified theoretical framework for optimizing such hybrid queries—a framework that could bridge the divide between the well-established relational optimization techniques and the evolving domain of graph query optimization.

Relational query optimization has historically leaned on the SPJ (selection-projection-join) skeleton, which provides a systematic approach for analyzing query complexity and devising logical equivalence transformation rules. This skeleton is pivotal in determining the optimal join order, ultimately underpinning the theoretical foundations for practical query optimization strategies. Paralleling these developments, significant strides have been made in optimizing graph pattern matching. Noteworthy among these are approaches leveraging backtracking-based algorithms, join-based algorithms, and hybrids thereof, with a distinct preference for join-based methods due to their alignment with relational operations.

In this paper, we endeavor to contribute to the formulation of the first converged relational/graph optimization framework. Four pivotal contributions have been made.

Firstly, we introduce the concept of \emph{Relational-to-Graph Mapping} (\rgmapping), which serves as the foundation for translating relational data models into property graph models as specified by SQL/PGQ’s CREATE GRAPH TABLE statements. Building on this, we define a new query skeleton named \spjm, which augments the traditional \spj query with a matching operator, thereby enhancing the framework to accommodate the complexities of converged relational and graph queries.

Secondly, we demonstrate that the \emph{matching operator} can be seamlessly transformed into an \spj query without loss of information, establishing the equivalency between optimizing an \spjm query and an \spj query using any standard relational optimizer. This finding is significant as it allows the use of existing relational optimizers to handle complex graph-related queries. Additionally, we exhibit that graph optimization techniques, when applied to the matching operator, benefit from a reduced search space and can effectively utilize graph indexes for efficiently computing adjacent edges and vertices, further streamlining the query optimization process.

Thirdly, synthesizing our theoretical insights, we propose\name, a \textbf{converged relational/graph optimization framework} aimed at optimizing \spjm queries. \name integrates a graph optimizer and a relational optimizer as its core components and includes a suite of heuristic rules designed to address the nuanced challenges presented by converged query scenarios.

Lastly, our contributions are substantiated by experimental results that underscore the efficacy of the proposed optimization framework. These results highlight the advantages of our approach in real-world applications, providing empirical evidence to support the theoretical advancements posited in our study.
