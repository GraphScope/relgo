...
R_k &= \gproject(\matching(\pattern_{k})) \\
    &= \widehat{\pi}_{A_1*}(\matching(\pattern_{e_k})) \Join_{attr_{V_o}}  \widehat{\pi}_{A_1*}(\matching(\pattern_{k-1})) \\
...
According to \reflem{graph-agnostic}, the graph-agnostic method must produce a sequence of joins among $n$ vertex relations and $m$ edge relations, which is equivalent to searching for the optimal join order among $n + m$ relations. We have the following theorem.
...
\begin{proof}
    We first estimate the number of possible join orders, and each join order is a logical plan of join operators.
    ...
    \modify{Given $n + m$ relations, we first construct a graph $\mathbb{G} = (V, E)$, where each relation corresponds to a vertex in $V$, and if there is a join condition between two relations, there is an edge between their corresponding vertices in $E$.
