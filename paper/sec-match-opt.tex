...
\begin{split}
R_{\lab(e)} & \evjoin R_{\lab(u_s)} = \{(t_e, t_s) \;|\; \\
  &  t_e \in R_{\lab(e)} \land t_s \in R_{\lab(u_s)} \land \lambda_{\lab(e)}^s(t_e) = t_s\}.
\end{split}
\end{equation}
...
In the graph view, given a vertex $v$, it is expected to be efficient to obtain its adjacent edges and neighbors. However, in the relational view, such adjacency relationships between vertices and edges are not directly stored in relations. Instead, \EVjoin operations (\refeq{ev-join}) must be performed to obtain these adjacency relationships. In GrainDB~\cite{graindb}, the authors introduced an indexing technique called pre-defined join to improve the performance of such join operations. As the pre-defined join essentially materializes the adjacency relationships, we consider it as a \emph{graph index} in this paper.
...
GrainDB introduces two kinds of indexes to the relational tables to efficiently process the join: the EV-index and the VE-index. The EV-index, shown in \reffig{graph-index}(a), is constructed by appending extra columns to the edge table $\relation{\text{Likes}}$. The column ``\text{pid\_rowid}'' stores the row number of the corresponding tuple in the source vertex table $\relation{\text{Person}}$, denoted as $\rowid(t_{p})$, where $t_{p} \in \relation{\text{Person}}$. Similarly, the column ``\text{mid\_rowid}'' stores the row number of the corresponding tuple in the target vertex table $\relation{\text{Message}}$, denoted as $\rowid(t_{m})$, where $t_{m} \in \relation{\text{Message}}$. These row numbers help quickly route a tuple $t_{l} \in \relation{\text{Likes}}$ to the joinable tuples $t_{p}$ and $t_{m}$ without the need for additional data structures like hash tables or sorting.

The VE-index, shown in \reffig{graph-index}(b), is created on the source vertex table $\relation{\text{Person}}$ for efficiently computing its ``liked messages''. For each tuple $t_{p} \in \relation{\text{Person}}$, the VE-index records the row numbers of the tuples $t_{l} \in \relation{\text{Likes}}$ and the corresponding $t_{m} \in \relation{\text{Message}}$ that are joinable with $t_{p}$. In the graph view, the VE-index maintains the adjacent edges and vertices of each person, treating ``Person-[Likes]->Messages'' as an edge of a property graph. GrainDB organizes the VE-index using the Compressed-Sparse-Row (CSR) structure.

It's straightforward to construct the graph indexes during the \rgmapping process using the \lstinline{CREATE PROPERTY GRAPH} statement. Given an edge relation $R_e$ and its associated vertex relations $R_{v_s}$ and $R_{v_t}$, the EV-index can be constructed on $R_e$ for each tuple $t_e \in R_e$ by including $\rowid(\lambda_e^s(t_e))$ and $\rowid(\lambda_e^t(t_e))$, which are the row numbers of the corresponding tuples in $R_{v_s}$ and $R_{v_t}$, respectively. Meanwhile, the VE-index can be constructed on $R_{v_s}$ for each tuple $t_{v_s} \in R_{v_s}$ by including the row numbers of all tuples $t_e \in R_e$ such that $\lambda_e^s(t_e) = t_{v_s}$, along with the row numbers of the corresponding tuples $t_{v_t} \in R_{v_t}$ such that $\lambda_e^t(t_e) = t_{v_t}$.
...
With the graph indices constructed, we can efficiently compute the \EVjoin operation in \refeq{ev-join} for both directions. When transforming the matching operator using the graph-agnostic method in \refeq{graph-agnostic} and optimizing the entire \spj query using a relational optimizer, we can replace all the remaining \EVjoin operations with GrainDB's pre-defined joins to leverage the graph indices. This approach serves as the baseline solution, which we refer to as GrainDB in our experiments.
However, as mentioned earlier, any pair of edge and vertex relations in an \EVjoin operation can be separated during the optimization process, preventing the utilization of graph indices. Even if certain rules are installed to prevent such separation, the resulting plan may still be suboptimal. This is because the plan for solving the matching operator reflects a naive method of edge-based join plan~\cite{lai2019distributed}, which is not worst-case optimal.

\subsubsection{The Graph-Aware Execution Plan}
\label{sec:join-matching-operator}
We delve into the physical implementation of the execution plan provided by the graph-aware method for solving $\matching(\pattern)$. The entry point of the plan is always matching a single-vertex pattern $\pattern_u$, which is one of the leaf nodes in the decomposition tree, and it selection is determined by the underlying optimizer (\refsec{optimizer}).

 The implementation of $\matching(\pattern_u)$ is straightforward: scanning the corresponding vertex relation $\relation{\lab(u)}$ and encoding each tuple as a graph vertex along with its id and label. There are several ways to assign the vertex identifier, but the simplest approach is to directly use the row number of the tuple in the relation. To ensure globally unique identifiers, we can encode the name of the relation as a prefix of the identifier, which can be easily implemented via bit shifting, while the name is encoded as an integer. Advanced encoding techniques are necessary for production use, but they are beyond the scope of this paper.

The plan is then constructed in a bottom-up manner, with the matching of $\pattern$ being the last stage. As shown in \reffig{match-decomposition}, there are three fundamental cases to consider when implementing the plan.

Case I: Solving $\matching(\pattern') = \matching(\pattern'_l) \gjoin_{u_s} \matching(\pattern_e)$, where $\pattern'_e$ is a single-edge pattern, and
$u_s$ is the source vertex\footnote{The expansion of edge can also be performed from the target vertex $u_t$, where the process is analogous.} in  $\pattern'_l$ from which the edge $e = (u_s, u_t)$ is expanded. Note that it's not possible that both $u_s$ and $u_t$ are in $\pattern'_l$, as it violates the fact that either $\pattern'_l$ is a single vertex or it is an induced sub-pattern.

%Taking a graph relation $\matching(\pattern'_l)$ as input, we devise the \expandx{vtag}{dir}{label} physical operator for computing the above join, with \kk{vtag} refers to the vertex
%in the graph relation $\matching(\pattern'_l)$ from which the edge can be expanded, \kk{dir} and \kk{label}
%denote the direction and label of the edge, respectively.


Case II: Solving $\matching(\pattern') = \matching(\pattern'_l) \gjoin_{V_o, E_o} \matching(\pattern(u:S))$, where $\pattern(u:S)$ is a complete star.


Case III: Solving $\matching(\pattern') = \matching(\pattern'_l) \gjoin_{V_o, E_o} \matching(\pattern'_r)$, where $\pattern_l'$ and $\pattern'_r$ are both intermediate patterns in the decomposition tree. The implementation of such a join is similar to a conventional relational join. The join is constrained to a natural join, where the join condition is simply the equality of the common vertices $V_o$ and edges $E_o$ between $\pattern_l'$ and $\pattern_r'$. During the implementation of the join, the identifiers of the vertices and edges can serve as the keys for comparison. Note that the input and output of the join are both graph relations, which will not
be projected into relational relations until the last stage that obtain the results $\matching(\pattern')$.
