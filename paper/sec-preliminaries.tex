...
A special kind of projection is graph projection $\widehat{\pi}_{prop} : \mathcal{J}_{GR} \rightarrow \mathcal{J}_{R}$ that maps graph relations to relations.
...
Besides, $GR$ is the graph relation that contains the data graphs that pattern matching is conducted on and $\mathcal{P}$ is the pattern.
