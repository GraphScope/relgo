...
When there exists a graph index, the implementation is handled via the physical \expandx{$u_s$}{$\lab(e)$} operator. For all $\tau \in \matching(\pattern'_l)$, recall that $\tau.u_s$ must record some graph vertex $v_s$ that matches $u_s$ in the pattern $\pattern'_l$. The process is as follows:
\begin{enumerate}
\item The \expand operator first locates the VE-index corresponding to $v_s$. From this index, it can obtain a list of row numbers corresponding to the edges incident to $v_s$. For example, if we are looking at the person $p_1$ in \reffig{graph-index}(b), the list $[1, 2]$ can be obtained. The row numbers of the target vertex tables are also obtained from the VE-index.
\item Using these row numbers, a list of adjacent edge/vertex tuples, e.g. $\{(e_{l_1}, v_{m_0}), (e_{l_2}, v_{m_1})\}$, can be computed.
\item The output tuples regarding the input $\tau$ are obtained by attaching each edge/vertex tuple to $\tau$, e.g. $((\tau, e_{l_1}, v_{m_0}), (\tau, e_{l_2}, v_{m_1}))$.
\end{enumerate}

Case III: Solving $\matching(\pattern') = \matching(\pattern'_l) \gjoin_{V_o, E_o} \matching(\pattern(u;S))$, where $\pattern(u;S)$ is a complete star. When there is no graph index, solving case II is simply a continuous joining of $|S|$ single-edge patterns.
When graph indexes are available, the implementation is handled via the physical \expandintersect~operator.
...
Moreover, we have $\lambda^s_{e_i}$ and $\lambda^\tau_{e_i}$ mapping tuples from $\relation{\lab(e_i)}$ to $\relation{\lab(v_i)}$, and from $\relation{\lab(e_i)}$ to $\relation{\lab(v_r)}$, respectively.
