...
\begin{definition}[Matching Operator, $\mathcal{M}$]
    Given a graph relation $GR$ representing the data graph and a pattern $\mathcal{P}$, the matching operator $\mathcal{M}: \mathcal{J}_{GR} \times \mathcal{Q} \rightarrow \mathcal{J}_{GR}$ returns a graph relation consisting of the subgraphs matching the pattern.
