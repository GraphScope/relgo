...
In this subsection, we analyze the gap between the two search space and conclude that the graph-aware approach is exponentially more efficient in terms of search space.
...
\begin{proof}
    ...
    %\end{equation*}
    \modify{
    Given a matching operator $\matching(\pattern)$, where $\pattern$ has $n$ vertices and $m$ edges, according to \reflem{spjm-to-spj}, the graph-agnostic method must produce a sequence of joins among the $n$ vertex relations and $m$ edge relations.
    Then, the search space of the graph-agnostic method is equivalent to the number of possible logical plans for the sequence of joins.
    The set of the possible logical plans is denoted by $\mathbb{PL}_r$.}
    
    In contrast, the graph-aware approach decomposes $\pattern$ and its search space equals the number of possible decomposition trees.
    Specifically, we further flatten the MMCs that are complete stars to single-vertex patterns containing only the root vertex.
    Under this method, the flattened decomposition trees are considered logical plans obtained by the graph-aware method, and they are denoted by $\mathbb{PL}_g$.
    Note that the main difference between the logical plans in $\mathbb{PL}_g$ and those in $\mathbb{PL}_r$ is that edge relations are not included in the plans in $\mathbb{PL}_g$.
    This is mainly due to the constraints imposed on the decomposition tree, which require each intermediate pattern to be an induced sub-pattern.
    Consequently, given any subset of vertices, enumerating edges among these vertices is not necessary.


    Then, we try to convert $\mathbb{PL}_g$ to $\mathbb{PL}_r$.
    Specifically, this can be achieved by adding edge relations to the logical plans in $\mathbb{PL}_g$. 
    For each logical plan $pl_g \in \mathbb{PL}_g$, a logical plan $pl_r \in \mathbb{PL}_r$ can be obtained as follows:
    Suppose $e = (u_s, u_t)$ is an edge in $\pattern$.
    By joining $\relation{\lab(e)}$ with $\relation{\lab(u_s)}$ and $\relation{\lab(u_t)}$ in $pl_g$ respectively, at least two new plans can be obtained.
    Since there are $m$ edge relations, at least $2^m$ logical plans in $\mathbb{PL}_r$ can be generated.
    Furthermore, the new plans generated from different logical plans $pl_g \in \mathbb{PL}_g$ are themselves distinct.
