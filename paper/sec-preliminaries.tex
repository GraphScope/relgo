...
\begin{example}
    \reffig{intro-rgmapping-example}(c) illustrates an example of an \spjm query, along with its corresponding SQL/PGQ expression. The query's objective is to identify pairs of persons who like the same message, know each other, and where one of them resides in Germany. As per the query, once the results of the matching operator are obtained (as described in \refex{matching}), a $\gproject$ operator is applied to extract attributes from all vertices, forming a relational table. Subsequently, the join, selection, and projection operators are used to compute the final results.
...
\begin{table}[ht]
   ...
   \label{tab:notations}
    \scalebox{0.9}{
        \begin{tabular}{|m{0.29\linewidth}|m{0.79\linewidth}|}
            \hline
            Notation                 & Definition                                                                                       \\
            \hline
