...
The relations that are mapped to vertices are referred to as vertex relations.
...
Additionally, we have two vertex relations $R_{v_s}$ and $R_{v_t}$, and the \emph{total} functions: (1) $\lambda_{e_i}^s: R_{e_i} \to R_{v_s}$ such that $v_s = \zeta_v(\lambda_{e_i}^s(t))$, and (2) $\lambda_{e_i}^t: R_{e_i} \to R_{v_t}$ such that $v_t = \zeta_v(\lambda_{e_i}^t(t))$. The relations that are mapped to edges are referred to as edge relations.
\end{itemize}
\end{definition}
