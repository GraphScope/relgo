...
In the graph view, given a vertex $v$, it is expected to be efficient to obtain its adjacent edges and neighbors. However, in the relational view, such adjacency relationships between vertices and edges are not directly stored in relations. Instead, EV-join operations (\refeq{ev-join}) must be performed to obtain these adjacency relationships. In GrainDB~\cite{graindb}, the authors introduced an indexing technique called pre-defined join to improve the performance of such join operations. As the pre-defined join essentially materializes the adjacency relationships, we consider it as a \emph{graph index} in this paper.
...
\subsubsection{Implementing Matching Operator}
