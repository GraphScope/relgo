...
It is easy to convert relational tables (relations) to a graph relation.
Given a set of relational tables $T$, some of tables can be regarded as vertices in a property graph (denoted by vertex tables), while others can be regarded as edges (denoted by edge tables).
For instance, according to the grammar of SQL/PGQ, with the following statement, a graph view can be generated.
\begin{lstlisting}
    CREATE PROPERTY GRAPH graph_view
        ...
        EDGE TABLES (
            Send KEY(pid, mid) SOURCE KEY(pid) REFERENCE Person(person_id) DESTINATION KEY(mid) REFERENCE Message(message_id)
        )
\end{lstlisting}
Specifically, rows in tables Person and Message are utilized as vertices and rows in table Send are edges, and a graph view is constructed.
It indicates that the attributes of graph relations, which are graph elements, come from rows of relational tables.
These vertices and edges can form a tuple representing a property graph.
Then, a graph relation that consists of this tuple is obtained.


A graph relation $GR$ can be represented as a set of property graphs, where each property graph in the set represents a tuple in $GR$ and the graph elements in the property graphs correspond to the attributes of $GR$.
To retrieve the labels of graph elements, the relational tables that the graph elements come from are obtained, and the names of the tables are the labels.
The process of retrieving the properties of the graph elements is similar to that of retrieving labels.
...
The relational matching algebra expands the domain and range of the operators in relational algebra and introduces three new operators, i.e., the conversion operator and the matching operator.
Specifically, the domain and range of the operators such as selection ($\sigma_d$), projection ($\pi_A$), and join ($\Join$) operators are expanded from $\mathcal{J}_R$ to $\mathcal{J}$.
...
Please note that the new projection operator can project a relation to a new relation or project a graph relation to a new graph relation, but cannot project between relations and graph relations.
Since the semantics of these operators remain unchanged, the description of these operators are omitted.
In the rest of this paper, the new operators are proposed in detail.

The conversion operator is variant of the projection operator.
Its difference from the projection operator mainly lies in the distinct domains and ranges.
The formal definition of the conversion operators is as follows.

\begin{definition}[Conversion Operator, $\widehat{\pi}_{prop}$]
    Given a graph relation $GR$, the conversion operator $\widehat{\pi}_{prop}: \mathcal{J}_{GR} \rightarrow \mathcal{J}_R$ projects the vertices and edges to a list of their properties (i.e., $prop$) and outputs a relation.
\end{definition}

\begin{definition}[Matching Operator, $\mathcal{M}$]
    Given a graph relation $GR$ representing the data graph and a pattern $\mathcal{P}$, the matching operator $\mathcal{M}: \mathcal{J}_{GR} \times \mathcal{Q} \rightarrow \mathcal{J}_{GR}$ return a graph relation consisting of the subgraphs matching the pattern.
...
Inspired by SPJ queries, for the property graph data model proposed in Sec.~\ref{sec:data-model}, we design a new class of queries named SPJCM under relational matching algebra.
The definition of an SPJCM query is as follows:
\begin{equation*}
    Q_m = \pi_A(\sigma_d(R_1 \Join \cdots \Join R_m \Join \widetilde{R}_1 \Join \cdots \Join \widetilde{R}_n))
\end{equation*}
where
\begin{equation*}
    \tilde{R}_i = \hat{\pi}_{prop_i}(\mathcal{M}(GR, \mathcal{P}))
\end{equation*}
is a relation generated by projecting the outputs of the matching operator $\mathcal{M}$, which is a graph relation.
Besides, $GR$ is the graph relation that contains the data graphs pattern matching is conducted on and $\mathcal{P}$ is the pattern.
