...
\stitle{Case III: Solving $\matching(\pattern') = \matching(\pattern'_l) \gjoin_{V_s, E_s} \matching(\pattern(u;V_s))$}, where pattern $\pattern(u;V_s)$ is a complete $k$-star. In this case, $\pattern'_l$ is a sub-pattern, and $\pattern(u;V_s)$ is a complete $k$-star pattern with a root vertex $u$ and leaf vertices $V_s = \{u_1, \ldots, u_k\}$.
...
\begin{example}
    ...
    The implementation of the decomposition tree follows a bottom-up order.
    Firstly, the join operation between $\matching(\pattern_3)$ and $\matching(\pattern_4)$ is implemented and it pertains to Case II, since the $\pattern_6$ is a single-edge pattern.
    ...
    Finally, $\matching(\pattern_1)$ and $\matching(\pattern_2)$ are joined together, where $\pattern_2 = (u_{m}, \{u_{p_1}, u_{p_2}\})$ is a complete $2$-star pattern.
    This join pretains to Case III and suppose the graph indexes are available.
    Take tuple $(v_{p_1}, e_{k_1}, v_{p_2})$ from $\matching(\pattern_1)$ as an example, where $u_{p_1}$ and $u_{p_2}$ in $\pattern$ match with $v_{p_1}$ and $v_{p_2}$, respectively.
    \expandedge~operator is first applied to obtain the adjacent edges and neighbors of $v_{p_1}$ and $v_{p_2}$, and the results are $(v_{p_1}, e_{k_1}, v_{p_2}, [e_{l_1}], [v_{m_1}])$ and $(v_{p_1}, e_{k_1}, v_{p_2}, [e_{l_2}, e_{l_3}], [v_{m_1}, v_{m_2}])$, respectively.
    Then, intersections are applied.
    As $\adj(v_{p_1}) \cap \adj(v_{p_2}) = [v_{m_1}]$, edges connected to $v_{m_1}$ are reserved and the returned tuple is $(v_{p_1}, e_{k_1}, v_{p_2}, [v_{m_1}])$.
    The tuple can be flattened to $(v_{p_1}, e_{k_1}, v_{p_2}, v_{m_1})$ if necesary.
\end{example}
