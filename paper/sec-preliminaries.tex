...
\begin{example}
    ...
    According to the query tree, after the results of the match operator are obtained as described in \refex{matching}, a specialized projection operator is applied to extract attributes from vertices and this operator returns a relational table (i.e., $R_{M}$ in \reffig{intro-rgmapping-example}(c)).
