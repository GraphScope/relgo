...
We have demonstrated in \reffig{framework-workflow}(c) a physical plan for matching the given triangle pattern.
\modify{In this physical plan, we initiate the process by identifying vertices labeled "Tom" through a scan of $\relation{Person}$. Subsequently, we sequentially apply the \expandvertex~and \expandintersect~operators to construct the triangle pattern. The procedure is completed by employing the \project* operator, which flattens the results into relational tuples for further analysis.
}
\todo{describe the physical plan}.
