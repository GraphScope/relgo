...
\begin{proof}
    ...
    which is equivalent to searching for the optimal join order among $n + m$ relations. Then, the search space equals the number of possible join orders. 
    
    Note that in this proof, we focus on path graphs, meaning the $n$ vertex relations and $m$ edge relations form a path.
    Then, let $c_p(k)$ denote the number of possible join orders for a path graph of length $k$, which means that the path graph has $k$ edges, we have:
    \begin{equation*}
        c_p(k) = 2\sum_{i=0}^{k-1}c_p(i)c_p(k-1-i),
    \end{equation*}
    where $c_p(0) = 1$. Using the generating function, we obtain:
    \begin{equation*}
        c_p(k) = \frac{2^k}{k+1}\binom{2k}{k}
    \end{equation*}
    ...
    Then, for the graph-aware transformation, let $s_p(k)$ denote the number of logical plans corresponding to a path graph of length $k$.
    Then, we have:
    \begin{equation*}
        \begin{split}
            s_p(k) & = 2\sum_{i=1}^{k-1}s_p(i)s_p(k-i) + 2s_p(k-1) \\
            & \leq 3\sum_{i=1}^{k-1}s_p(i)s_p(k-i), \text{when } k \geq 2\\
        \end{split}
    \end{equation*}
    where $s_p(0) = 1$ and $s_p(1) = 2$.
    
    Let
    \begin{equation*}
        s'_p(k) = 3\sum_{i=1}^{k-1}s'_p(i)s'_p(k-i), k \geq 2,
    \end{equation*}
    and by organizing the equation, we obtain
    \begin{equation*}
        s''_p(k + 1) = 3\sum_{i=0}^{k}s''_p(i)s''_p(k-i), k \geq 0,
    \end{equation*}
    which means that $s'_p(k)$ corresponds to $s''_p(k - 1)$.
    Note that $s''_p(0) = s'_p(1) = 2$.

    Therefore, we have 
    \begin{equation*}
        s''_p(k) = \frac{2 * 6^k}{k+1}\binom{2k}{k}, \text{ and } 
        s_p(k) \leq \frac{2 * 6^{k - 1}}{k}\binom{2k - 2}{k - 1},
    \end{equation*}

    Please note that the length of a pattern graph consisting of $n$ vertex relations and $m$ edge relations is $m = n - 1$.

    Therefore, using Stirling's formula, we have 
    \begin{equation*}
        \frac{c_p(m+n-1)}{s_p(m)} = \frac{c_p(2m)}{s_p(m)} \geq O((\frac{8}{3})^m)
    \end{equation*}
...
Additionally, we compared the optimizer’s query optimization time. For our comparison, Apache Calcite, a generic relational optimization framework, served as the optimizer for the graph-agnostic method. In contrast, our \name, which is implemented based on Calcite, acts as the optimizer for the graph-aware method. Both \name and Calcite are implemented in Java, utilizing the VolcanoPlanner of Calcite with default rules. Notably, we did not consider any heuristics for either Calcite or \name, providing a fair comparison and a clear demonstration of the reduced search space.
The optimization time was evaluated using the queries in our experiment (details in \refsec{evaluation}).
If the optimization for a query does not finish within 10 minutes, the process is stopped early, and the time cost of such optimization is recorded as 10 minutes. The results, shown in \reffig{exp-optimization-sf30} and \reffig{exp-optimization-job}, demonstrate the superiority of \name, which outperforms Calcite signifcantly in query optimization speed. For instance, when querying $\text{IC}_{5-1}$, the optimization time using \name is more than \( 10^4 \) times faster compared to Calcite. Additionally, \name can complete optimizing almost all queries within 10-100 milliseconds, with a maximum time cost of 210 milliseconds, while Calcite often exceeds the 10-minute limit, especially for JOB queries.
