...
As illustrated in \reffig{framework-overview}, the core workflow of the \name framework consists of two main components: a \emph{graph optimizer} and a \emph{postposition relational optimizer}. The graph optimizer is responsible for handling the matching operator in an \spjm query. Once the matching operator has been optimized, the postposition relational optimizer takes over to optimize the remaining relational operators in the query.
...
\begin{itemize}
\item When $\gjoin$ is implemented as a \hashjoin, the cost is calculated as the product of the cardinalities of the two relations being joined, i.e., $\cost(\gjoin) = |\matching(\pattern'_l)| \times |\matching(\pattern'_r)|$.
...
\subsubsection{Postposition Relational Optimizer}
