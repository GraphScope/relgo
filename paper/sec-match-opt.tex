...
Given that edges may connect to common vertices, it's obvious that there are some redundant joins in \refeq{graph-agnostic}. Removing these redundancies results in a sequence of joins corresponding to the number of vertices and edges in the pattern graph, as illustrated in the following example.
...
\todo{add single-vertex pattern as an MMC.}
...
\begin{proof}
    ...
    \modify{Given $n + m$ relations, we first construct a graph $G_R = (V_{G_R}, E_{G_R})$, where each relation corresponds to a vertex in $V_{G_R}$, and if there is a join condition between two relations, there is an edge between their corresponding vertices in $E_{G_R}$.
    Then, each possible logical plan forms a spanning tree in $G_R$, and different logical plans may form the same tree.
    Therefore, the number of possible logical plans can be computed by obtaining all the possible logical plans corresponding to the spanning trees in $G_R$.}
...
Based on \refthm{complexity-of-volcano} and \refthm{complexity-of-graph-aware}, we can ultimately obtain the following theorem.

\begin{theorem}
    \label{thm:compare-search-space}
    The search space of optimization with the graph-aware method is exponentially smaller than that with the graph-agnostic method.
\end{theorem}
\begin{proof}
    \begin{equation*}
...
\comment{In practical implementation, efficient pruning strategies can be employed to trim the search space. 
We conducted experiments on Linked Data Benchmark Council (LDBC) social network benchmark \cite{ldbc_snb} with using the dataset with scale factor of 30.
The utilized queries are those in \cite{graindb}. 
In the experiments, Calcite was used as a typical volcano optimizer, while our \name employed the Graph-aware method. 
The experimental results of optimizing queries using Calcite and \name are shown in \reffig{exp-optimization-sf30}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=.6\linewidth]{./figures/exp/optimization_sf30.pdf}
    \caption{Comparison of the optimization time.}
    \label{fig:exp-optimization-sf30}
\end{figure}

The experimental results show that the optimization time of Calcite is exponentially longer than that of \name. Additionally, it is worth paying attention to the results of three queries: IC1-1, IC1-2, and IC1-3. IC1-2 is obtained by adding a node and an edge to the pattern graph of IC1-1, while IC1-3 is derived by adding a node and an edge to IC1-2. It can be observed that as the number of nodes and edges increases, the time cost for Calcite also grows exponentially, which is consistent with the \refthm{complexity-of-volcano}. In this case, the optimization time for RelGO does not show significant change with the increase of nodes and edges. This is because RelGO implements an efficient pruning strategy in its execution, further reducing the optimization complexity to be linearly related to the number of edges in $\searchgraph_\pattern$.}
