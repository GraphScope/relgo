...
which is equivalent to searching for the optimal join order among $n + m$ relations. The following lemma\footnote{The proofs of \reflem{complexity-of-volcano}
and \reflem{complexity-of-graph-aware} are in the full paper~\cite{full_paper}.}
...
\begin{example}
    ...
    Then, $\matching(\pattern_3) \widehat{\Join}_{\{v_{p_1}\}} \matching(\pattern_4)$ pretains to Case I.
    The join operator is implemented as a natural join with constraints on $v_{p_1}$.

    Finally, $\matching(\pattern_1)$ and $\matching(\pattern_2)$ are joined together, where $\pattern_2 = (v_{m_2}, \{v_{m_1}, v_{p_2}\})$ is a complete $2$-star pattern.
    Without loss of generality, let $m_1$ and $p_1$ be vertices in $\matching(\pattern_1)$ that matches $v_{m_1}$ and $v_{p_2}$, respectively.
    Suppose the graph indexes are available, the physical operator \expandintersect~is applied.
    Specifically, \expandedge~and \getvertex~operators are applied to obtain the adjacent edges and neigbors of $m_1$ and $p_1$, and the obtained tuples are $(m_1, [e_{l_1}, e_{l_2}], [v_{m_1}, v_{m_2}])$ and $(p_1, [e_{s_1}, e_{s_2}], [v_{m_2}, v_{m_3}])$.
    Then, these tuples are intersected and the result is $(m_1, p_1, [(v_{m_2}, e_{l_1}, e_{s_1})])$.
    Then, the return tuple is $(\tau, [(v_{m_2}, e_{l_1}, e_{s_1})])$.
    $m_1$ and $p_1$ are included in $\tau$.
\end{example}
