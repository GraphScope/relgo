...
Let $\mathcal{J} = 2^{2^{V \cup E \cup D}}$ be the domain of the sets of records, $\mathcal{J}_D = 2^{2^{D}}$ be the domain of the sets of records whose attributes are not vertices or edges. 
...
Specifically, the domain and range of the operators such as selection ($\sigma_d$), projection ($\pi_A$), and join ($\Join$) operators are expanded from $\mathcal{J}_D$ to $\mathcal{J}$.
In other words, in relational matching algebra, these operators act on graph relations, and their outputs are also graph relations.
Since the semantics of the operators that already exist within relational algebra remain unchanged, the description of these operators are omitted.
In the rest of this paper, the matching operator is proposed in detail.

\begin{definition}[Matching Operator, $\mathcal{M}$]
    Given two graph relations $V$ and $E$ consisting of the vertices and edges in a graph respectivey and a pattern $\mathcal{P}$, the matching operator $\mathcal{M}: \mathcal{J} \times \mathcal{J} \times \mathcal{Q} \rightarrow \mathcal{J}$ return a graph relation consisting of the subgraphs matches the pattern.
...
Inspired by SPJ queries, for the property graph data model proposed in Sec.~\ref{sec:data-model}, we design a new class of queries named SPJG under graph relational algebra.
The definition of SPJG is as follows:
\begin{equation*}
    Q_g = \pi_A(\sigma_d(R_1 \Join \cdots \Join R_m \Join \widetilde{R}_1 \Join \cdots \Join \widetilde{R}_n))
\end{equation*}
where
\begin{equation*}
    \tilde{R}_i = \hat{\pi}_{col_i}(\mathcal{M}(V, E, \mathcal{P}))
\end{equation*}
is a relation generated by projecting the outputs of the matching operator $\mathcal{M}$, which is a graph relation.
Besides, $V$ and $E$ are graph relations of vertices and edges respectively, and $\mathcal{P}$ is the pattern.

Regarding the SPJG problem, the main difference between the aforementioned four types of optimizers lies in their search space when optimizing the physical implementation of the matching operator.
