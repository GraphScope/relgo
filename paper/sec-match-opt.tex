...
\subsubsection{Comparison of Optimization Time}
\revise{To facilitate a more intuitive comparison of the optimization speed between graph-agnostic and graph-aware approaches, we implement the optimizer named \name based on the graph-aware method and select Apache Calcite,, a widely-used optimizer employing graph-agnostic techniques. We then compare their optimization time. This comparison employs two different benchmarks (namely, LDBC-SNB and JOB) and is conducted on two datasets. Detailed descriptions of these benchmarks and datasets will be provided in \refsec{evaluation}.
Please note that the codes of \name and Calcite are both written in Java, and VolcanoPlanner of Calcite with default rules is leveraged.
