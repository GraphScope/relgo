...
For \revise{each edge relation $R_e$, there should be two corresponding vertex relations $R_{p_s}$ and $R_{p_t}$. $R_e$ must have two foreign keys referencing the primary keys of $R_{p_s}$ and $R_{p_t}$, respectively, as shown in \reffig{er-diagram}.
Note that the cardinality type does not necessarily have to be many-to-many; in fact, it can be of any cardinality type (e.g., one-to-one) ({\color{red} comment(Robin): this Cardinality type is strange}).
