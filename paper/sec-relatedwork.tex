...
Various studies of query optimization for relational databases were proposed to find the optimal join order \cite{IbarakiK84,optimize-nested-vldb-1986,Haffnerjoinorder,data-dependency-join}.
For example, 
...
\revise{Recently, researchers have attempted to incorporate wco joins into generated plans to better handle queries containing cycles and reduce the size of intermediate results~\cite{Aberger2016Sigmod,free-join}.
...
Join-based algorithms have been developed for distributed environments. These algorithms use cost estimation to optimize join order, with binary-join algorithms\cite{lai2015scalable, lai2019distributed} estimating costs using random graph models and worst-case-optimal join algorithms~\cite{ammar2018distributed} ensuring a worst-case upper bound on the cost. \revise{Hybrid approaches\cite{mhedhbi2019optimizing, huge, jin2023cidr} adaptively select between binary and wco joins based on the lower cost}. 
...
\revise{Hence, DuckPGQ loses the opportunity to optimize the query from a graph query perspective}.
