...
Note \revise{that \filterrule is a global optimization rule 
because pushing the predicate into the matching operator does not always result in a better plan. For instance, given $\matching(\pattern)$ and assuming the query represented by $\pattern$ is} 
\begin{lstlisting}
