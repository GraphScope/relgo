...
The workflow of \name optimization framework is illustrated in \reffig{framework-overview}. The framework consists of two primary components: the \emph{graph optimizer} and the \emph{postposition converged optimizer}, which
are applied sequentially to an \spjm query, for producing the optimized execution plan.

\subsubsection{The Graph Optimizer}
The graph optimizer focuses on optimizing the matching operator, leveraging the graph-aware method introduced in \refsec{graph-aware}. In this work, we adopt the graph optimization techniques developed in \glogs~\cite{GLogS}, which is the state-of-the-art graph optimizer that considers real-life graph pattern matching in a property graph.

According to~\cite{GLogS}, a data structure called \glogue~ is constructed, which is similar to the graph structure $\searchgraph_{\pattern}(V, E)$ that we introduced in \refsec{compare-search-space} for analyzing the search space of the graph-aware method. The main difference is that \glogue~not only maintains sub-patterns of a specific pattern but also include all patterns with a number of vertices within a threshold ($k=3$). Each vertex $\pattern'$ in the G\glogue~ maintains a numeric value corresponding to the cardinality of the pattern in the graph, denoted as $|\matching(\pattern')|$. Given that computing $|\matching(\pattern')|$ directly on the original graph is costly, \glogs proposes to first sparsify the graph as $G'$ and then use $|\matching_{G'}(\pattern')|$ as an estimate value. In this work, for computing \glogue, we randomly
sample a set of vertex and edge relations during \rgmapping, and use the techniques in~\cite{gart}
to construct the sparsified graph.

Let the cost of an execution plan $\Phi$ for computing $\matching(\pattern)$ be $\cost_\Phi(\pattern)$. The graph optimizer aims to find an optimal execution plan such that $\cost_\Phi(\pattern)$ is minimized. Clearly, this can be solved using dynamic programming~\cite{GLogS}.

Consider an execution plan $\Phi$ where $\matching(\pattern') = \matching(\pattern'_l) \gjoin \matching(\pattern'_r)$ is computed. We must have:
\[
\cost_{\Phi}(\pattern') = \cost_{\Phi_l}(\pattern'_l) + \cost_{\Phi_r}(\pattern'_r) + \cost(\gjoin),
\]
where $\Phi_l$ and $\Phi_r$ are the execution plans for computing $\matching(\pattern'_l)$ and $\matching(\pattern'_r)$, respectively, and $\cost(\gjoin)$ is the cost of the join operation. In this case, finding the optimal execution plans for $\matching(\pattern'_l)$ and $\matching(\pattern'_r)$ are the optimal subproblems.

In \refsec{join-matching-operator}, we discussed the physical implementation of three types of join operators: hash-join, \expand, and \expandintersect. When a graph index is available, we can compute the cost of the $\gjoin$ operator, denoted as $\cost(\gjoin)$, based on the specific implementation used, as described in [18]. The cost calculation differs for each case:
\begin{itemize}
\item When $\gjoin$ is implemented as a hash-join, the cost is calculated as the product of the cardinalities of the two relations being joined, i.e., $\cost(\gjoin) = |\matching(\pattern'_l)| \times |\matching(\pattern'_r)|$.
\item If $\gjoin$ is implemented using the \expand~ operator, the cost is calculated based on the cardinality of $\matching(\pattern'_l)$ and the average degree of the graph, namely $|\matching(\pattern'_l)| \times \overline{d}$.
\item When $\gjoin$ is implemented using the \expandintersect~ operator, the cost is calculated based on the cardinality of $\matching(\pattern'_l)$ and the average intersection size of the neighbors of the vertices being intersected, which is maintained on the corresponding edge from $P'_l$ to $\pattern'_l$ in \glogue.
\end{itemize}

In the absence of a graph index, hash-join is used for the entire plan, where the cost is simply the product of the cardinalities of the two relations being joined.
