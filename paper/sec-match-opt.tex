...
The optimization of the matching operator, after transforming it using graph-agnostic and graph-aware methods, is equivalent to performing pure relational optimization and graph optimization, respectively. We compare the search space of the two optimizations rather than their time complexity using advanced optimizers for several reasons. First, some optimizers, such as the one proposed in~\cite{Haffnerjoinorder}, involve heuristics that make it challenging to perform a rigorous analysis of their behavior. Second, the abundance of relational optimizers~\cite{astarjoin,chenjoinorder, Haffnerjoinorder}, makes it difficult to select a single optimizer as the gold standard for comparison. Finally, the techniques employed by relational optimizers may also be applicable to their graph counterparts, potentially resulting in similar improvements in both cases.

According to \reflem{spjm-to-spj}, the graph-agnostic method must produce a sequence of joins among $n$ vertex relations and $m$ edge relations, which is equivalent to searching for the optimal join order among $n + m$ relations. The following lemma establishes a lower bound on the search space complexity for this problem:
...
In contrast, when the graph-aware method is applied, the search space is equivalent to the number of possible decomposition trees. Despite the numerous works proposed to optimize graph pattern matching~\cite{huge,GLogS,mhedhbi2019optimizing}, to the best of our knowledge, the search space of this optimization problem has not been thoroughly analyzed. The following lemma provides an upper bound on the search space complexity for the graph-aware method:
...
\begin{remark}
    \label{re:search-space}
    It is worth noting that the search space after applying the graph-aware method, as given in \reflem{complexity-of-graph-aware}, does not depend on $m$, the number of edges in the pattern. This is due to the constraints imposed on the decomposition tree, which require each intermediate pattern to be an induced sub-pattern. Consequently, given any subset of vertices, enumerating edges among these vertices is not necessary. To further verify the findings in \refthm{compare-search-space}, we conducted a micro-benchmark experiment using a path graph with $n + 1$ vertices and $n$ edges. We programmed an enumerator to explore the search space of both the graph-agnostic and graph-aware methods while varying $n$. The results, shown in \reffig{exp-search-space}, confirm the significant difference in search space size between the two methods.
\end{remark}
...
\[
    (\tau, \adj^E(v_i), \cdots, \adj^E(v_k), \bigcap_{1 \leq i \leq k}\adj(v_i)).
\]
\end{enumerate}
