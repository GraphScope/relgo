...
GrainDB introduces two kinds of indexes to the relational tables to efficiently process the join. The first kind of index, called EV-index, as shown in \reffig{graph-index}(a), is constructed by appending an extra column named ``\text{pid\_rowid}'' to the table $\relation{\text{Likes}}$, corresponding to the row number in the table $\relation{\text{Person}}$. When conducting the join, the row number helps quickly route a tuple of $\relation{\text{Likes}}$ to the joinable tuple of $\relation{\text{Person}}$ without the extra maintenance of a hash table in a hash-join or sorting in a merge-sort-join implementation. Similarly, the ``\text{mid\_rowid}'' column is added to efficiently join $\relation{\text{Likes}}$ with $\relation{\text{Message}}$.

The second kind of index, called VE-index, as shown in \reffig{graph-index}(b), is created on the table $\relation{\text{Person}}$ for efficiently computing its ``liked messages''. Specifically, for each person, the VE-index records the row numbers of the tuples in $\relation{\text{Likes}}$ and the corresponding $\relation{\text{Message}}$ that are joinable with the person. In the graph view, the VE-index essentially maintains adjacent edges and vertices of each person if viewing ``Person-[Likes]->Messages'' as an edge of a property graph. The VE-index used in GrainDB is organized in the structure of Compressed-Sparse-Row (CSR).
