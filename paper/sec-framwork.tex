...
\revise{In the absence of a graph index, \hashjoin~ is used for the entire plan of the matching operator, because the joins connecting vertices and their adjacent edges are always far from cross product.
...
\revise{After years of research and exploration, relational optimization has become a mature field, yielding numerous significant results \cite{Chaudhuri98,Haffnerjoinorder}. Since relational optimization is not the focus of this paper and various relational optimizers can be seamlessly plugged into \name for this purpose, we will only discuss how relational and graph optimizers are utilized concurrently within \name}.
...
\revise{Note that \filterrule is a global optimization rule relevant to both graph optimization and relational optimization., while \joinfuserule is a local optimization rule specifically for graph optimization.
The effectiveness of these two rules is validated in \refsec{experiment-opt}. In addition to these simple yet effective rules, we have also designed several other rules, such as \degreerule that avoids the final join for aggregate operations and \subtaskremoverule that converts some where conditions to anti joins and semi joins.
}
...
\revise{
    While low-order statistics primarily focus on the cardinalities of relational tables representing vertices and edges, high-order statistics also include the frequencies of sub-patterns with at least three vertices, which aids in more accurate cost estimation. It is important to note that \name remains functional with only low-order statistics, but the efficiency of the generated plan may decrease due to less accurate cost estimation.
}
