...
\modify{It is difficult to precisely compute the time complexity of optimizing with the volcano optimizer due to the fact that it reduces the search space using heuristics and cost estimate.
Therefore, in this work, we focus on the worst case when all the heuristic strategies fail and almost all the possible plans are visited.
In this case, we consider the lower bound of time complexity to be approximately equal to that of the search space and compare it to the upper bound of the graph-aware method, thereby enhancing the constructiveness of the comparison. Although there may exist other relational optimizers~\cite{xx} that exhibit greater efficiency in terms of search space, our study remains sound within a generic context.}
...
Determining the exact number of edges in $\searchgraph_\pattern$ for an arbitrary pattern is non-trivial. However, since we are studying the upper bound, we can consider the worst-case scenario. Given a sub-pattern $\pattern'$ consisting of $1 < i < n$ vertices, \modify{the number of induced subgraphs of $\pattern'$ is at most $\Sigma_{1 \leq j \leq i - 1}\binom{i}{j} = 2^i - 2$}, which constrains the maximum number of edges that $\pattern'$ can connect to.
\modify{Besides, the number of edges that $\pattern' \in V_1$ can connect to is always 1.}
