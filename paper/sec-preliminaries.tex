...
Suppose a tuple $\tau$ in relation $R$ is mapped to a vertex $v \in V$ (resp.~edge $e = (v_s, v_t) \in E$), it is assigned an ID $\id(v)$ that matches the name of $R$ (resp.~$\id(e)$), a label $\lab(v)$ (resp.~$\lab(e)$), and attributes $v.attr*$ that mirror the attributes $attr*$ of $\tau$ (resp.~$e.attr*$).
Relations mapped to vertices and edges in the graph are referred to as vertex relations and edge relations, respectively}.

\begin{figure}
    \centering
    \includegraphics[width=.7\linewidth]{./figures/erd.pdf}
    \caption{An example of ER diagram representing the relationships between vertex relations and edge relations.}
    \label{fig:er-diagram}
\end{figure}

For \revise{each edge relation $R_e$, there should be two corresponding vertex relations $R_{p_s}$ and $R_{p_t}$, statisfying that $R_e$ have two foreign keys referencing the primary keys of $R_{p_s}$ annd $R_{p_t}$ respectively as shown in \reffig{er-diagram}.
Note that the cardinality type does not necessarily have to be many-to-many; in fact, it can be of any cardinality type (e.g., one-to-one).
We further define two total functions: $\lambda_{q_i}^s: R_{q_i} \to R_{p_s}$ and $\lambda_{e}^t: R_{q_i} \to R_{p_t}$, which maps a tuple $\tau \in R_{e}$ to a tuple $\tau_s \in R_{p_s}$ and a tuple $\tau_t \in R_{p_t}$, respectively.
The vertices $v_s$ and $v_t$ corresponding to $\tau_s$ and $\tau_t$, respectively, are obtained by applying the vertex mapping function}.
...
\begin{figure*}
    \centering
    \includegraphics[width=.95\linewidth]{./figures/rgmapping.pdf}
