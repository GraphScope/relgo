...
Note \revise{that \filterrule is a global optimization rule because pushing the predicate into the matching operator does not always yield a better plan and requires careful evaluation.
However, since it is generally effective, we implement it as a local optimization in \name for better optimization performance}.
\iffalse
